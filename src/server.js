...
app.get('/api/frogs', async (req, res) => {
  try {
    const frogs = await all('SELECT * FROM frogs ORDER BY created_at DESC');
    res.json(frogs);
  } catch (err) {
...
app.post('/api/frogs', async (req, res) => {
  try {
    ...
    const frog = await get('SELECT * FROM frogs WHERE id = ?', [result.id]);
...
app.get('/api/frogs/:id', async (req, res) => {
  try {
    const frog = await get('SELECT * FROM frogs WHERE id = ?', [req.params.id]);
    res.json(frog || {});
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.put('/api/frogs/:id', async (req, res) => {
  try {
...
app.delete('/api/frogs/:id', async (req, res) => {
  try {
    await run('DELETE FROM frogs WHERE id = ?', [req.params.id]);
...
app.get('/api/habitats/:frogId', async (req, res) => {
  try {
...
app.put('/api/habitats/:habitatId', async (req, res) => {
  try {
    const { wallpaper } = req.body;
    await run('UPDATE habitats SET wallpaper = ? WHERE id = ?', [wallpaper, req.params.habitatId]);
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.get('/api/habitats/:habitatId/items', async (req, res) => {
  try {
    const items = await all('SELECT * FROM habitat_items WHERE habitat_id = ?', [req.params.habitatId]);
    res.json(items);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post('/api/habitats/:habitatId/items', async (req, res) => {
  try {
    ...
    const item = await get('SELECT * FROM habitat_items WHERE id = ?', [result.id]);
...
app.delete('/api/habitats/:habitatId/items/:itemId', async (req, res) => {
  try {
    await run('DELETE FROM habitat_items WHERE id = ?', [req.params.itemId]);
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post('/api/breeding', async (req, res) => {
  try {
    ...
    const parent1 = await get('SELECT * FROM frogs WHERE id = ?', [parent1_id]);
    const parent2 = await get('SELECT * FROM frogs WHERE id = ?', [parent2_id]);
    ...
    const breeding = await get('SELECT * FROM breeding WHERE id = ?', [result.id]);
...
app.post('/api/breeding/:id/hatch', async (req, res) => {
  try {
    const breeding = await get('SELECT * FROM breeding WHERE id = ?', [req.params.id]);
    ...
    const parent1 = await get('SELECT * FROM frogs WHERE id = ?', [breeding.parent1_id]);
    const parent2 = await get('SELECT * FROM frogs WHERE id = ?', [breeding.parent2_id]);
    ...
    const baby = await get('SELECT * FROM frogs WHERE id = ?', [babyResult.id]);
